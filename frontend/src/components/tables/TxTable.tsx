...
import { useDeleteTx } from '@/hooks/service/tx/useDeleteTx';
...
const TxTable = (): JSX.Element => {
  ...
  const { isPending, error, data, isFetching, isRefetching, isLoading, refetch, isSuccess } = useGetManyTx(
    (page - 1) * SKIP,
  );
  ...
  const handleChangePage = (direction: number) => {
    setPage((prev) => {
      ...
      if (direction > 0 && (!data || data.length === 0) && (!queryTxMutation.data || queryTxMutation.data.length === 0))
        return prev;
      return nextPage;
    });
  };
  ...
  useEffect(() => {
    ...
    queryTxMutation.data,
    data,
    queryTxMutation.isSuccess,
    queryTxMutation.isPending,
    isSuccess,
    isPending,
    isLoading,
    isFetching,
            ...
            {items && items.length > 0 ? (
              items.map((item: TxProps, i: number) => (
                ...
                >
                  ...
                  <TableCell align="left">{item.cr ? 'Entrada' : 'Saída'}</TableCell>
                  ...
                  <TableCell align="right">
                    {item.tdate ? new Date(item.tdate + 'T00:00:00').toLocaleDateString() : '-'}
                  </TableCell>
