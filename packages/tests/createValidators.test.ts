import { createCatSchema } from '../src/validators/zodSchemas/create/createCatValidator';
import { createCfSchema } from '../src/validators/zodSchemas/create/createCfValidator';
import { createCpSchema } from '../src/validators/zodSchemas/create/createCpValidator';
import { createCrSchema } from '../src/validators/zodSchemas/create/createCrValidator';
import { createPartnerSchema } from '../src/validators/zodSchemas/create/createPartnerValidator';
import { createTcfSchema } from '../src/validators/zodSchemas/create/createTcfValidator';
import { createTcrSchema } from '../src/validators/zodSchemas/create/createTcrValidator';
import { createTcpSchema } from '../src/validators/zodSchemas/create/createTcpValidator';
import { createTxSchema } from '../src/validators/zodSchemas/create/createTxValidator';
import { createTokenSchema } from '../src/validators/zodSchemas/create/createTokenValidator';
import { createUserSchema } from '../src/validators/zodSchemas/create/createUserValidator';
import { createUserRepPwdSchema } from '../src/validators/zodSchemas/create/createUserRepPwdValidator';
import { PartnerType } from '../src/dtos/utils/enums';
...
describe('Schema Tests', () => {
  describe('createCatSchema', () => {
    ...
    it('should fail validation if name is too long', () => {
      expect(() => createCatSchema.parse({ ...validData, name: 'a'.repeat(101) })).toThrow(
        'Nome pode ter no máximo 100 caracteres',
      );
    });

    it('should pass validation if description is within the limit', () => {
      expect(() => createCatSchema.parse({ ...validData, description: 'Descrição válida' })).not.toThrow();
    });

    it('should fail validation if description is too long', () => {
      expect(() => createCatSchema.parse({ ...validData, description: 'a'.repeat(256) })).toThrow(
        'Descrição pode ter no máximo 255 caracteres',
      );
    });

    it('should pass validation if obs is within the limit', () => {
      expect(() => createCatSchema.parse({ ...validData, obs: 'Observação válida' })).not.toThrow();
    });

    it('should fail validation if obs is too long', () => {
      expect(() => createCatSchema.parse({ ...validData, obs: 'a'.repeat(256) })).toThrow(
        'Observação pode ter no máximo 255 caracteres',
      );
    });

    it('should fail validation if user is not a valid UUID', () => {
      expect(() => createCatSchema.parse({ ...validData, user: 'invalid-uuid' })).toThrow('Informar o usuário');
    });
  });
  ...
  describe('createCfSchema', () => {
    ...
    it('should fail validation if number is too long', () => {
      expect(() => createCfSchema.parse({ ...validData, number: 'a'.repeat(11) })).toThrow(
        'Número precisa ter no máximo 10 caracteres',
      );
    });

    it('should pass validation if ag is within the limit', () => {
      expect(() => createCfSchema.parse({ ...validData, ag: '1234567890' })).not.toThrow();
    });

    it('should fail validation if ag is too long', () => {
      expect(() => createCfSchema.parse({ ...validData, ag: 'a'.repeat(11) })).toThrow(
        'Agência precisa ter no máximo 10 caracteres',
      );
    });

    it('should pass validation if bank is within the limit', () => {
      expect(() => createCfSchema.parse({ ...validData, bank: 'Banco exemplo limitado a 30' })).not.toThrow();
    });

    it('should fail validation if bank is too long', () => {
      expect(() => createCfSchema.parse({ ...validData, bank: 'a'.repeat(31) })).toThrow(
        'Banco precisa ter no máximo 30 caracteres',
      );
    });

    it('should fail validation if balance is missing', () => {
      expect(() => createCfSchema.parse({ ...validData, balance: undefined })).toThrow('valor é obrigatório');
    });

    it('should fail validation if type is not a valid UUID', () => {
      expect(() => createCfSchema.parse({ ...validData, type: 'invalid-uuid' })).toThrow('Informar o tipo');
    });

    it('should fail validation if user is not a valid UUID', () => {
      expect(() => createCfSchema.parse({ ...validData, user: 'invalid-uuid' })).toThrow('Informar o usuário');
    });

    it('should pass validation if obs is within the limit', () => {
      expect(() => createCfSchema.parse({ ...validData, obs: 'Observação válida' })).not.toThrow();
    });

    it('should fail validation if obs is too long', () => {
      expect(() => createCfSchema.parse({ ...validData, obs: 'a'.repeat(256) })).toThrow(
        'Observação pode ter no máximo 255 caracteres',
      );
    });
  });
  ...
  describe('createCpSchema', () => {
    ...
    it('should fail validation if value is missing', () => {
      expect(() => createCpSchema.parse({ ...validData, value: undefined })).toThrow('valor é obrigatório');
    });
    ...
    it('should fail validation if type is not a valid UUID', () => {
      expect(() => createCpSchema.parse({ ...validData, type: 'invalid-uuid' })).toThrow('Informar o tipo');
    });

    it('should fail validation if user is not a valid UUID', () => {
      expect(() => createCpSchema.parse({ ...validData, user: 'invalid-uuid' })).toThrow('Informar o usuário');
    });

    it('should fail validation if supplier is not a valid UUID', () => {
      expect(() => createCpSchema.parse({ ...validData, supplier: 'invalid-uuid' })).toThrow('Informar o fornecedor');
    });

    it('should pass validation if obs is within the limit', () => {
      expect(() => createCpSchema.parse({ ...validData, obs: 'Observação válida' })).not.toThrow();
    });

    it('should fail validation if obs is too long', () => {
      expect(() => createCpSchema.parse({ ...validData, obs: 'a'.repeat(256) })).toThrow(
        'Observação pode ter no máximo 255 caracteres',
      );
    });
  });
  ...
  describe('createCrSchema', () => {
    ...
    it('should fail validation if value is missing', () => {
      expect(() => createCrSchema.parse({ ...validData, value: undefined })).toThrow('valor é obrigatório');
    });
    ...
    it('should fail validation if type is not a valid UUID', () => {
      expect(() => createCrSchema.parse({ ...validData, type: 'invalid-uuid' })).toThrow('Informar o tipo');
    });

    it('should fail validation if user is not a valid UUID', () => {
      expect(() => createCrSchema.parse({ ...validData, user: 'invalid-uuid' })).toThrow('Informar o usuário');
    });

    it('should fail validation if customer is not a valid UUID', () => {
      expect(() => createCrSchema.parse({ ...validData, customer: 'invalid-uuid' })).toThrow('Informar o cliente');
    });

    it('should pass validation if obs is within the limit', () => {
      expect(() => createCrSchema.parse({ ...validData, obs: 'Observação válida' })).not.toThrow();
    });

    it('should fail validation if obs is too long', () => {
      expect(() => createCrSchema.parse({ ...validData, obs: 'a'.repeat(256) })).toThrow(
        'Observação pode ter no máximo 255 caracteres',
      );
    });
  });
  ...
  describe('createPartnerSchema', () => {
    ...
    it('should fail validation if name is too long', () => {
      expect(() => createPartnerSchema.parse({ ...validPfData, name: 'a'.repeat(101) })).toThrow(
        'Nome pode ter no máximo 100 caracteres',
      );
    });

    it('should pass validation if cod has non-digit characters (they should be removed)', () => {
      expect(() => createPartnerSchema.parse({ ...validPfData, cod: '004.801.710-84' })).not.toThrow();
      expect(() => createPartnerSchema.parse({ ...validPjData, cod: '33.113.309/0001-47' })).not.toThrow();
    });
    ...
    it('should fail validation if user is not a valid UUID', () => {
      expect(() => createPartnerSchema.parse({ ...validPfData, user: 'invalid-uuid' })).toThrow('Informar o usuário');
    });

    it('should pass validation if obs is within the limit', () => {
      expect(() => createPartnerSchema.parse({ ...validPfData, obs: 'Observação válida' })).not.toThrow();
    });

    it('should fail validation if obs is too long', () => {
      expect(() => createPartnerSchema.parse({ ...validPfData, obs: 'a'.repeat(256) })).toThrow(
        'Observação pode ter no máximo 255 caracteres',
      );
    });
  });
  ...
  describe('createTcfSchema', () => {
    ...
    it('should fail validation if name is too short', () => {
      expect(() => createTcfSchema.parse({ name: 'ab' })).toThrow('Nome pode ter ao menos 3 caracteres');
    });

    it('should fail validation if name is too long', () => {
      expect(() => createTcfSchema.parse({ name: 'a'.repeat(256) })).toThrow('Nome pode ter no máximo 255 caracteres');
    });
  });
  ...
  describe('createTcpSchema', () => {
    ...
    it('should fail validation if name is too short', () => {
      expect(() => createTcpSchema.parse({ name: 'ab' })).toThrow('Nome pode ter ao menos 3 caracteres');
    });

    it('should fail validation if name is too long', () => {
      expect(() => createTcpSchema.parse({ name: 'a'.repeat(256) })).toThrow('Nome pode ter no máximo 255 caracteres');
    });
  });
  ...
  describe('createTcrSchema', () => {
    ...
    it('should fail validation if name is too short', () => {
      expect(() => createTcrSchema.parse({ name: 'ab' })).toThrow('Nome pode ter ao menos 3 caracteres');
    });

    it('should fail validation if name is too long', () => {
      expect(() => createTcrSchema.parse({ name: 'a'.repeat(256) })).toThrow('Nome pode ter no máximo 255 caracteres');
    });
  });
  ...
  describe('createTokenSchema', () => {
    ...
    it('should fail validation if cpf is in invalid format', () => {
      expect(() => createTokenSchema.parse({ ...validData, cpf: '12345' })).toThrow('CPF em formato inválido.');
    });
  ...
  describe('createTxSchema', () => {
    ...
    it('should fail validation if value is missing', () => {
      expect(() => createTxSchema.parse({ ...validDataCp, value: undefined })).toThrow('valor é obrigatório');
    });

    it('should fail validation if user is not a valid UUID', () => {
      expect(() => createTxSchema.parse({ ...validDataCp, user: 'invalid-uuid' })).toThrow('Informar o usuário (UUID)');
    });
    ...
    it('should pass validation if description is within the limit', () => {
      expect(() => createTxSchema.parse({ ...validDataCp, description: 'Descrição válida' })).not.toThrow();
    });

    it('should fail validation if description is too long', () => {
      expect(() => createTxSchema.parse({ ...validDataCp, description: 'a'.repeat(101) })).toThrow(
  ...
  describe('createUserRepPwdSchema', () => {
    ...
    it('should fail validation if passwords do not match', () => {
      expect(() => createUserRepPwdSchema.parse({ ...validData, confirmPwd: 'SenhaDiferente' })).toThrow(
    ...
    it('should fail validation if cpf is invalid', () => {
      expect(() => createUserRepPwdSchema.parse({ ...validData, cpf: '123' })).toThrow('CPF inválido');
    });

    it('should fail validation if pwd does not meet criteria', () => {
      expect(() =>
        createUserRepPwdSchema.parse({ ...validData, pwd: 'senhafraca', confirmPwd: 'senhafraca' }),
      ).toThrow();
    });
  });
  ...
  describe('createUserSchema', () => {
    ...
    it('should fail validation if cpf is invalid', () => {
      expect(() => createUserSchema.parse({ ...validData, cpf: '123' })).toThrow('CPF inválido');
    });
  });
});
